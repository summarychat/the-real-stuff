...
def message_json(chat):
    session = sessionmaker()
    result = session.query(Message).filter(Message.channel == chat).order_by("timestamp")[-50:]
    return json.dumps([row.diction() for row in result])

@app.route('/events/<chat>', methods=['GET'])
def event_json(chat):
    session = sessionmaker()
    inter = session.query(Event).filter(Event.channel == chat).order_by("timestamp")[-5:]
